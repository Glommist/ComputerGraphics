...
void Toolbar::layout_mode(Scene& scene)
{
    if (ImGui::BeginTabItem("Layout")) {
        ...
        if (selected_object != nullptr) {
            ...
            float x_angle_radians = (float)radians(x_angle);
            float y_angle_radians = (float)radians(y_angle);
            float z_angle_radians = (float)radians(z_angle);
            Quaternionf qx(cos(x_angle_radians / 2), sin(x_angle_radians / 2), 0, 0);
            Quaternionf qy(cos(y_angle_radians / 2), 0, sin(y_angle_radians / 2), 0);
            Quaternionf qz(cos(z_angle_radians / 2), 0, 0, sin(z_angle_radians / 2));
